...
import glob
import datetime
from config import fields as f_map #importing from the folder config
import logging
...
def write_deployment(path, deployment):
    ...
    try:
        ...
        out_file.close()
        xmlfolder = os.path.join(root_directory,"xmls")#lets also export to a folder
        if not os.path.isdir(xmlfolder):
            os.mkdir(xmlfolder)
        out_test = open(os.path.join(xmlfolder,deployment['camera_deployment_id']+".xml"), 'w')
